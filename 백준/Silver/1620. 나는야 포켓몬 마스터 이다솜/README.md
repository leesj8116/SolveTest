...
1. 포켓몬 이름 -> 도감 번호 변환을 얼마나 빨리 할 것인가
2. 포켓몬 이름/도감 번호 입력을 어떻게 구분할 것인가
   이 두가지에 대해 묻는 문제였습니다.

처음에는 단순하게 List 하나로 관리하면서 `List.indexOf()`로 접근했더니 당연히 시간 초과로 실패했습니다.
문자열 검색 결과를 별도로 담는 Map을 사용하면서 정답 까지는 받았지만, 결과 시간이 너무 오래걸려 잘못했다고 판단했습니다.

다른 사람들의 결과를 참고하여 3차 수정까지 하고나니 정상적인 풀이로 보였습니다.
문제의 난이도는 어렵지 않다고 생각했는데, 결과가 만족스럽지 않아서 아쉬웠습니다. 꾸준히 풀면서 개선하면 좋겠습니다..

---

### 성능 요약

메모리: 39180 KB, 시간: 540 ms
